...
- `differentiable_mk_of_finSupp`: finitely-supported `l1Weighted.mk` is differentiable
...
This is the key API for proving differentiability through `l1Weighted.mk`. -/
lemma differentiable_mk_of_finSupp {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E]
    (f : E → ℕ → ℝ) (M : ℕ) (hsupp : ∀ a n, M < n → f a n = 0)
    (hf : ∀ a, l1Weighted.Mem ν (f a))
    (hd : ∀ k : Fin (M + 1), Differentiable ℝ (fun a => f a k)) :
    Differentiable ℝ (fun a => l1Weighted.mk (f a) (hf a)) := by
  -- mk(f a) = Σ_k single_CLM k (f a k): both agree at each toSeq index.
  have hsum_eq : ∀ a, l1Weighted.mk (f a) (hf a) =
      ∑ k : Fin (M + 1), single_CLM (ν := ν) (k : ℕ) (f a k) := by
    intro a; apply l1Weighted.ext; intro n
    simp only [l1Weighted.mk_apply, toSeq_finset_sum, single_CLM_apply]
  ...
  intro a; show DifferentiableAt ℝ (fun a => l1Weighted.mk (f a) (hf a)) a
  rw [show (fun a => l1Weighted.mk (f a) (hf a)) =
...
/-- Fderiv of finitely-supported `l1Weighted.mk` vanishes on tail modes.
...
lemma fderiv_mk_of_finSupp_toSeq_tail {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E]
    ...
    (hd : ∀ k : Fin (M + 1), Differentiable ℝ (fun a => f a k))
    (x h : E) (n : ℕ) (hn : M < n) :
    toSeq ((fderiv ℝ (fun a => l1Weighted.mk (f a) (hf a)) x) h) n = 0 := by
  -- The function is identically zero at mode n > M (since f a n = 0)
  -- So the coordinate function `a ↦ toSeq(mk(f a)) n` is constant 0
  have hzero_fn : (fun a => toSeq (l1Weighted.mk (f a) (hf a)) n) = fun _ => 0 :=
    funext fun a => by simp [hsupp a n hn]
  -- fderiv of constant = 0
  have hfderiv_zero : (fderiv ℝ (fun a => toSeq (l1Weighted.mk (f a) (hf a)) n) x) h = 0 := by
    rw [hzero_fn]; simp
  -- Connect: toSeq((fderiv (mk ∘ f) x) h) n = fderiv(a ↦ toSeq(mk(f a)) n) x h
  have hchain : toSeq ((fderiv ℝ (fun a => l1Weighted.mk (f a) (hf a)) x) h) n =
      (fderiv ℝ (fun a => toSeq (l1Weighted.mk (f a) (hf a)) n) x) h := by
    have : (fun a => toSeq (l1Weighted.mk (f a) (hf a)) n) =
        ⇑(toSeq_CLM (ν := ν) n) ∘ (fun a => l1Weighted.mk (f a) (hf a)) := rfl
