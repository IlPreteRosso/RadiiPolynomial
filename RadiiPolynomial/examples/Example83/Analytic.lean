...
Mirrors `Example81/Analytic.lean`. Per-example obligations are minimal:
`defect_norm_lt_one` (Z₀ ≈ 10⁻¹⁵ < 1) and the `banachField ↔ f` bridge
(which is `rfl` because Example83 defines `f` via `evalBanach`).
-/
